...
import io.lemonlabs.uri.{Url => CoreUri}
  ...
  implicit def instance(implicit endpoint: ApiEndpoint, client: Client[IO]): JsonApiClient = new JsonApiClient {
    ...
    override def one[A](id: String, include: Set[String] = Set.empty)(implicit pt: PathToId[A],
      ...
      for {
        ...
        response <- pathOne(config.uri.withPath(pt.self(id).path), include)
      } yield response
    ...
    override def pathOne[A](path: CoreUri, include: Set[String] = Set.empty)(
      ...
      val request = for {
        ...
        uri <- IO.fromEither(
          org.http4s.Uri
            .fromString((config.uri.withPath(path.path) ? ("include" -> mkIncludeString(include))).toString))
    ...
    override def post[A, Response](entity: A, include: Set[String] = Set.empty)(
      ...
      val request = for {
        config <- endpoint.config
        uri    <- IO.fromEither(org.http4s.Uri.fromString((config.uri / pt.entity(entity)).toString))
